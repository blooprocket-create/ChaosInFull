...
export async function GET(_req: Request, { params }: IdCtx) {
	...
	const row = await prisma.enemyTemplate.findUnique({ where: { id: params.id } });
...
export async function PATCH(req: Request, { params }: IdCtx) {
	...
	try {
		const row = await prisma.enemyTemplate.update({ where: { id: params.id }, data });
		return NextResponse.json({ ok: true, row });
	} catch {
...
export async function DELETE(_req: Request, { params }: IdCtx) {
	try { await assertAdmin(); } catch { return NextResponse.json({ ok: false, error: "forbidden" }, { status: 403 }); }
	try {
		await prisma.enemyTemplate.delete({ where: { id: params.id } });
