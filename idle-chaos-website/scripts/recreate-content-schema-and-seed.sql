-- Recreate content schema (safe to drop) and seed data for Idle Chaos
-- This targets only content tables referenced by seed and gameplay (no user/account tables).

-- Note: Not creating pgcrypto here to avoid permission issues in serverless Postgres

BEGIN;
-- Ensure any DEFERRABLE constraints are deferred during this transaction
SET CONSTRAINTS ALL DEFERRED;

-- 1) Drop in dependency order (children first)
DROP TABLE IF EXISTS "QuestRewardItem" CASCADE;
DROP TABLE IF EXISTS "PortalDef" CASCADE;
DROP TABLE IF EXISTS "SpawnConfig" CASCADE;
DROP TABLE IF EXISTS "DropEntry" CASCADE;
DROP TABLE IF EXISTS "DropTable" CASCADE;
DROP TABLE IF EXISTS "EnemyTemplate" CASCADE;
DROP TABLE IF EXISTS "Quest" CASCADE;
DROP TABLE IF EXISTS "NpcDef" CASCADE;
DROP TABLE IF EXISTS "ZoneDef" CASCADE;
DROP TABLE IF EXISTS "ItemDef" CASCADE;
...
CREATE TABLE "DropEntry" (
  ...
  itemid       TEXT NOT NULL REFERENCES "ItemDef"(id) ON DELETE RESTRICT,
...
CREATE TABLE "Quest" (
  ...
  objectivecount      INTEGER,
  givernpcid          TEXT REFERENCES "NpcDef"(id) ON DELETE SET NULL,
  nextquestid         TEXT REFERENCES "Quest"(id) ON DELETE SET NULL DEFERRABLE INITIALLY DEFERRED,
  rewardgold          INTEGER DEFAULT 0,
  rewardexp           INTEGER DEFAULT 0,
  rewardminingexp     INTEGER DEFAULT 0,
  rewardfishingexp    INTEGER DEFAULT 0,
  rewardcraftingexp   INTEGER DEFAULT 0,
  requiresquestid     TEXT REFERENCES "Quest"(id) ON DELETE SET NULL DEFERRABLE INITIALLY DEFERRED,
...
-- 3) Seed content (mirrors scripts/seed.ts)
...
INSERT INTO "EnemyTemplate" (id, name, level, basehp, damage, expbase, goldmin, goldmax, tags) VALUES
  ('slime', 'Regular Slime', 1, 30, 5, 5, 1, 3, 'slime'),
  ('slime_big', 'Big Slime', 2, 55, 8, 8, 2, 4, 'slime,big'),
  ('slime_epic', 'Epic Slime', 3, 80, 12, 12, 3, 5, 'slime,epic'),
  ('slime_epic_big', 'Epic Big Slime', 4, 120, 18, 18, 4, 7, 'slime,epic,big'),
  ('slime_boss', 'Boss Slime', 8, 450, 60, 60, 25, 60, 'slime,boss'),
  ('ghost', 'Ghost', 5, 110, 20, 20, 4, 8, 'undead,ghost'),
  ('ghost_epic', 'Epic Ghost', 7, 180, 32, 32, 6, 12, 'undead,ghost,epic'),
  ('skeleton', 'Skeleton', 6, 140, 25, 25, 5, 10, 'undead,skeleton'),
  ('skeleton_epic', 'Epic Skeleton', 8, 220, 38, 38, 7, 14, 'undead,skeleton,epic'),
  ('zombie', 'Zombie', 5, 120, 22, 22, 4, 9, 'undead,zombie'),
  ('zombie_epic', 'Epic Zombie', 7, 190, 34, 34, 6, 13, 'undead,zombie,epic')
ON CONFLICT (id) DO UPDATE SET
  name = EXCLUDED.name,
...
-- Stage quests without cross-row references to avoid FK order issues
INSERT INTO "Quest" (id, name, description, objectivetype, objectivetarget, objectivecount, givernpcid, rewardgold, rewardexp, minlevel) VALUES
  ('tutorial_kill_slimes_5', 'Can you punch?', 'Kill 5 slimes in the field.', 'KILL', 'slime', 5, 'grimsley', 500, 250, 1)
ON CONFLICT (id) DO UPDATE SET
  name = EXCLUDED.name,
  description = EXCLUDED.description,
...
INSERT INTO "Quest" (id, name, description, objectivetype, objectivetarget, objectivecount, rewardcraftingexp, rewardexp, minlevel) VALUES
  ('tutorial_craft_copper_dagger', 'Can you craft?', 'Craft one copper dagger at the workbench.', 'CRAFT', 'copper_dagger', 1, 150, 150, 1)
ON CONFLICT (id) DO UPDATE SET
  name = EXCLUDED.name,
  description = EXCLUDED.description,
  objectivetype = EXCLUDED.objectivetype,
...
-- Now wire the references
UPDATE "Quest" SET nextquestid = 'tutorial_craft_copper_dagger' WHERE id = 'tutorial_kill_slimes_5';
UPDATE "Quest" SET requiresquestid = 'tutorial_kill_slimes_5' WHERE id = 'tutorial_craft_copper_dagger';
...
ON CONFLICT ON CONSTRAINT portal_unique DO NOTHING;
