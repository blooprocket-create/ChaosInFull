import { addWindowListener } from '../../shared/cleanupManager.js';
import { ITEM_DEFS } from '../../data/items.js';
import { RECIPE_DEFS } from '../../data/recipes.js';
import { updateQuestProgress } from '../../data/quests.js';
import { onSkillLevelUp } from '../../data/talents.js';
...
function renderRecipes(scene) {
    ...
    const craftableCache = new Map();
    const levelCache = new Map();
    const getCraftableFor = (key, recipe) => {
        if (!recipe) return 0;
        if (!craftableCache.has(key)) craftableCache.set(key, getCraftableCount(scene, recipe));
        return craftableCache.get(key) || 0;
    };
    const getLevelFor = (key, recipe) => {
        if (!recipe) return 0;
        if (!levelCache.has(key)) levelCache.set(key, recipe.reqLevel || 1);
        return levelCache.get(key) || 1;
    };
    keys.sort((a,b) => {
        const ra = defs[a] || {};
        const rb = defs[b] || {};
        const craftA = getCraftableFor(a, ra) > 0;
        const craftB = getCraftableFor(b, rb) > 0;
        if (craftA !== craftB) return craftA ? -1 : 1;
        const lvlA = getLevelFor(a, ra);
        const lvlB = getLevelFor(b, rb);
...
export function openFurnace(scene) {
    ...
    // Register with cleanup manager so it is removed automatically on scene shutdown/destroy
    try { addWindowListener(scene, 'keydown', scene._furnaceEscHandler); } catch (e) { try { window.addEventListener('keydown', scene._furnaceEscHandler); } catch (e2) {} }
