...
export class GloamwayBastion extends Phaser.Scene {
    ...
    _ensureDialogueOverlay() {
        ...
        if (!document.getElementById('dialogue-animations-bastion')) {
            const style = document.createElement('style');
            style.id = 'dialogue-animations-bastion';
            style.textContent = `
                @keyframes fadeIn {
                    from { opacity: 0; }
                    to { opacity: 1; }
    ...
    _renderDialogue(title, bodyNodes, optionConfigs) {
        ...
        portrait.style.cssText = `
            width: 48px;
            height: 48px;
            background: linear-gradient(135deg, rgba(92,134,255,0.3), rgba(60,100,200,0.2));
            border: 2px solid rgba(92,134,255,0.5);
            border-radius: 50%;
            display: flex;
            align-items: center;
            justify-content: center;
        ...
        `;
        portrait.textContent = '🔮'; // Mother Lumen icon
        ...
        `;
        npcTitle.textContent = 'Keeper of Paths';
        ...
        (optionConfigs || []).forEach(opt => {
            ...
            btn.style.cssText = `
                padding: 12px 16px;
                border: 2px solid rgba(92,134,255,0.4);
                border-radius: 6px;
                cursor: pointer;
                background: linear-gradient(90deg, rgba(92,134,255,0.8), rgba(60,100,200,0.7));
                color: #fff;
                font-family: inherit;
                font-size: 0.95em;
                font-weight: 600;
            ...
            btn.onmouseover = function() {
                this.style.background = 'linear-gradient(90deg, rgba(110,150,255,0.9), rgba(80,120,230,0.8))';
                this.style.borderColor = 'rgba(92,134,255,0.6)';
                this.style.transform = 'translateX(4px)';
                this.style.boxShadow = '0 4px 12px rgba(0,0,0,0.6)';
            };
            btn.onmouseout = function() {
                this.style.background = 'linear-gradient(90deg, rgba(92,134,255,0.8), rgba(60,100,200,0.7))';
                this.style.borderColor = 'rgba(92,134,255,0.4)';
    ...
    _createDialogueParagraph(text) {
        ...
        p.style.cssText = `
            ...
            border-left: 3px solid rgba(92,134,255,0.3);
            border-radius: 4px;
        `;
        p.textContent = text;
        return p;
    ...
    _buildObjectiveList(questDef, progressStates) {
        ...
        container.style.cssText = `
            ...
            border-radius: 6px;
            border: 1px solid rgba(92,134,255,0.2);
        `;
        ...
        progressStates.forEach((obj) => {
            ...
            const desc = obj.description || `Complete ${required} ${obj.type}`;
            ...
            objectiveItem.innerHTML = `
                <div style="display: flex; align-items: center; gap: 6px; margin-bottom: 4px;">
                    <span style="color: ${isComplete ? '#4ade80' : '#5c86ff'}; font-weight: bold; font-size: 0.9em;">${isComplete ? '✓' : '○'}</span>
                    <span style="font-size: 0.9em; color: ${isComplete ? '#4ade80' : '#d4c5b9'};">${desc}</span>
                </div>
                <div style="display: flex; align-items: center; gap: 8px; padding-left: 22px;">
                    ...
                    ">
                        <div style="
                            height: 100%;
                            width: ${percent}%;
                            background: ${isComplete ? 'linear-gradient(90deg, #4ade80, #22c55e)' : 'linear-gradient(90deg, #5c86ff, #4070ff)'};
                            box-shadow: 0 0 8px ${isComplete ? 'rgba(74,222,128,0.4)' : 'rgba(92,134,255,0.4)'};
                        "></div>
                    </div>
                    <span style="
                        font-size: 0.85em;
